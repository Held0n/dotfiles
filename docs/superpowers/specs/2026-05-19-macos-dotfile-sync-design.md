...
- 新 macOS 装机后，能通过**一句命令**把以下内容恢复到与旧机一致的状态：
  ...
  - zsh 配置（`~/.zshrc`、`~/.p10k.zsh`、`~/.config/zsh/*.zsh`）
...
- **GitHub CLI /** **`gh auth login`** **gh auth login** 等需要交互登录的步骤的自动化。
...
│
├── zsh-macos/                     # stow 包：mac 上 zsh 的全部
│   ├── .zshrc                     # → ~/.zshrc（入口，按顺序显式 source 下面 5 个）
│   └── .config/zsh/
│       ├── env.zsh                # env / PATH / brew compinit
│       ├── zinit.zsh              # zinit installer + 插件加载 + zstyle
│       ├── prompt.zsh             # source ~/.p10k.zsh
│       ├── functions.zsh          # j() + git_branch + list_branch_with_description
│       └── extras.zsh             # 所有 alias + focus trap + bun + 额外 PATH
├── zsh-ubuntu/                    # stow 包（保留位置，本次不动；TODO 按 zsh-macos 模式重做）
│   └── .zshrc
│
├── p10k-macos/
│   └── .p10k.zsh                  # → ~/.p10k.zsh
...
└── secrets/
    └── ssh/                       # age -p 密文
        ...
        ├── id_rsa.pub.age
        ├── id_rsa_myself.age
        ├── id_rsa_myself.pub.age
        ├── coral_ed25519.age
        ├── coral_ed25519.pub.age
        ├── starrocks-dev.age
        ├── starrocks-dev.pub.age
...
| `script/git_functions.zsh`        | 合并进 `zsh-macos/.config/zsh/functions.zsh` | 顶层 `script/` 目录彻底取消         |
| `script/`（顶层目录）             | 删除                                      | 内容并入 `zsh-macos/.config/zsh/`   |
...
| 3 | stow packages     | `cd "$REPO_ROOT" && stow -t "$HOME" zsh-macos p10k-macos git ssh-config` + 检测存在的 `tmux nvim vim` 包后追加 stow                                               | stow 默认行为：撞到已有非软链文件就报错退出，**不自动覆盖、不自动备份** |
...
- **passphrase 一旦忘记，仓库里所有** **`.age`** **.age** **文件都解不开**。
...
把现有 `macos/zsh/zshrc` 拆成 6 个文件，全部归属 `zsh-macos/` 包：

| 文件 | 内容 |
|---|---|
| `zsh-macos/.zshrc` | 入口，按顺序显式 source 下面 5 个 |
| `zsh-macos/.config/zsh/env.zsh` | `TERM`、`LLVM*_HOME`、`JAVA_HOME`、`CLASSPATH`、PATH 拼接、`unsetopt LIST_BEEP`、Homebrew compinit |
| `zsh-macos/.config/zsh/zinit.zsh` | zinit installer 块 + 所有 zinit plugin 加载 + zstyle 补全配置 |
| `zsh-macos/.config/zsh/prompt.zsh` | `[[ ! -f ~/.p10k.zsh ]] \|\| source ~/.p10k.zsh` |
| `zsh-macos/.config/zsh/functions.zsh` | `j()` 函数 + `git_branch` / `list_branch_with_description`（从原 `script/git_functions.zsh` 合并） |
| `zsh-macos/.config/zsh/extras.zsh` | alias（`ls=exa`、`clear`、`jump`、`sr-dev`）、focus reporting trap、`bun` 初始化、追加 `~/.local/bin` 和 `mysql-client` PATH |
...
显式列 5 行 source 比 `for f in *.zsh; do source` 直观，文件名也不必带数字前缀。

#### 剥离 ClickHouse

新文件**不包含**以下原 zshrc 内容（彻底删）：
...
`extras.zsh` 中保留两个 alias，`jump` 的 pem 路径从 `/Users/heldon/...` 改为 `~/.ssh/`，与 decrypt-ssh 落点对齐：
...
- Ubuntu 侧（`zsh-ubuntu/`、`p10k-ubuntu/`）：保留位置，本次不动，留 TODO。后续 Ubuntu 重组时按 `zsh-macos` 同样的 6 文件模式做一份，`zinit.zsh` / `prompt.zsh` / `functions.zsh` 内容跟 mac 几乎一样，但不强行共享（接受短期重复，需要时再开 `zsh-common/` 包）。
...
| 文件 | 类型/注释 | 用途 |
|---|---|---|
| `id_rsa` / `.pub` | RSA, `heldon@MAC-L16PKW0RQ7` | 公司 key，`~/.ssh/config` 中所有 `10.4.x.x` 主机（dr02-dr05、redck-k8s-1-*）都用它 |
...
另外 `/Users/heldon/zhanghongtai-jumpserver.pem`（当前不在 `~/.ssh/`）也一并加密入库，由 decrypt-ssh 落到 `~/.ssh/`，`alias jump` 路径同步对齐。
...
1. **REDpass UUID 段（`# REDpassSSHAgent BEGIN/END` 之间）**：拷到仓库后**手动从仓库文件中删除**。新 mac 装 REDpass 后由它自动追加新 UUID 的段。
2. **`Include ~/.orbstack/ssh/config`**：保留原样。新 mac 没装 OrbStack 时 ssh 会有 warning 但仍能跑；装上 OrbStack 后自动生效。
3. **`# coral-mutagen:start/end` 段**：保留原样搬过去。Coral 工具初次连接时若发现已有段会复用 / 更新 Pod IP；如果强制覆盖整段也无害（stow 软链会让覆盖落回仓库文件，commit 前 review 一下）。
...
4. `j somedir` 能用（说明 `zsh-macos` 包 stow 成功、`.zshrc` source 链路完整）。
5. `git_branch` 能用（验证从原 `script/git_functions.zsh` 合并到 `functions.zsh` 的内容生效）。
6. `which jump`、`which sr-dev` 出 alias 定义；`jump` 能连（视网络）。
7. `ls -la ~/.zshrc ~/.p10k.zsh ~/.gitconfig ~/.ssh/config` 都是软链且指向仓库内文件。
8. `ls -la ~/.ssh/id_* ~/.ssh/*.pem` 是实文件、权限 600。
...
7. 删除旧顶层目录：`init.sh`、`link.sh`、`macos/`、`script/`、`ubuntu/`（确认重组完成、新结构跑通后再删；Ubuntu 的旧 zshrc 在删除前先复制到 `zsh-ubuntu/.zshrc` 占位，避免数据丢失）。
